...
func (sio *SerialIO) handleLine(logger *zap.SugaredLogger, line string) {
	...
	// split on pipe (|), this gives a slice of numerical strings between "0" and "max_slider_value (1023)"
	...
	for sliderIdx, stringValue := range splitLine {
		...
		if sliderIdx == 0 && number > sio.deej.config.MaxSliderValue {
			sio.logger.Debugw("Got malformed line from serial, ignoring", "line", line)
			return
		}

		// map the value from raw to a "dirty" float between 0 and 1 (e.g. 0.15451...)
		dirtyFloat := float32(number) / float32(sio.deej.config.MaxSliderValue)
